...
public class FileController {
	...
	public String upload(String title, MultipartFile myFile, HttpServletRequest request) {
		...
		/*
		 * 입력한 제목은 title에 들어있고, 업로드 된 파일에 대한 정보는 myFile 객체에 들어 있다.
		 * 따라서 myFile 객체의 메소드를 활용해서 업로드 처리에 필요한 정보를 얻어내고
		 * 어떤 동작을 수행하면 된다.
		 * 참고) 일반적인 웹서버 application은 클라이언트가 파일 업로드를 해오면
		 * 운영체제의 임시폴더에 이상한(긴) 파일명으로 일단 저장해 놓는다.
		 * 일정 시간 이후에 임시 폴더에 저장되었던 파일은 운영체제가 자동으로 지우거나 비운다.
		 * 따라서 해당 파일을 사용하기 위해서는 원하는 위치로 이동(or copy)시켜야 한다.
		 */
	...
	public String upload(FileDto dto, HttpServletRequest request) {
		// FileDto의 필드에 MultipartFile 객체의 참조값이 들어있다.
	...
	public Map<String, Object> imageUpload(MultipartFile image, HttpServletRequest request) {
		...
		//"imagePath" 라는 키값으로 업로드 된 이미지를 요청할 수 있는 경로를 담아서
		map.put("imagePath", "/resources/upload/"+saveFileName);
		//리턴해주면 {"imagePath":"xxx"} 형식의 json 문자열이 응답한다.
		return map;
	}
}
